...
train_data_dir = os.path.join(project_root, 'dataset/motor_only/train/')
valid_data_dir = os.path.join(project_root, 'dataset/motor_only/valid')
annotations_dir = os.path.join(project_root, 'dataset/motor_only/annotations')
supported_fmt = ['.jpg', '.JPG']


def class_text_to_int(row_label):
    if row_label == 'motor':  # 标注文件里面的标签名称
        return 1
    else:
        return None
...
def xml_to_csv(data_list):
    ...
    for data in data_list:
        ...
        for member in root.findall('object'):
            ...
            if name is not None and not class_text_to_int(name.text):
                print(f'Found {name.text}, skipped')
                continue
            if name.text != 'motor':
                continue
            bndbox = member.find('bndbox')
            if bndbox is None:
                print(f'No bndbox found for object:{name.text}, skipped')
                continue
            value = (data['image'],
                     width,
                     height,
